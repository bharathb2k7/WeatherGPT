...
// Helper to fetch Open-Meteo Weather Forecast
...
// High reliability Google Translate fallback for Telugu
async function translateToTeluguFallback(text: string): Promise<string | null> {
  try {
    ...
    const url = `https://translate.googleapis.com/translate_a/single?client=gtx&sl=auto&tl=te&dt=t&q=${encodeURIComponent(
      cleanText
    )}`;
    const res = await fetch(url);
    if (!res.ok) return null;
    const data = await res.json();
    const teluguText = data[0]?.map((chunk: any) => chunk[0]).join("") || null;
    return teluguText;
  } catch (err) {
    console.warn("Telugu translation fallback error:", err);
    return null;
  }
}
...
function synthesizePlainLanguageAdvisory(
  ...
  if (queryLower.includes("pesticide") || queryLower.includes("spray") || queryLower.includes("crop")) {
  ...
  if (queryLower.includes("fish") || queryLower.includes("sea") || queryLower.includes("boat") || queryLower.includes("marine")) {
  ...
  if (queryLower.includes("commute") || queryLower.includes("rain") || queryLower.includes("evening") || queryLower.includes("umbrella")) {
    const isEvening = queryLower.includes("evening") || queryLower.includes("night");
  ...
  if (queryLower.includes("storm") || queryLower.includes("alert") || queryLower.includes("heatwave") || queryLower.includes("cyclone")) {
...
// API: Standalone Telugu translation endpoint
app.post("/api/translate/telugu", async (req, res) => {
  ...
  try {
    ...
    // Fallback to high-accuracy Google Translate
  }
...
app.post("/api/chat", async (req, res) => {
  ...
  const isTeluguRequested =
    language === "te" ||
    message.toLowerCase().includes("telugu") ||
    message.includes("తెలుగు") ||
    message.toLowerCase().includes("translate to telugu") ||
    message.toLowerCase().includes("in telugu");
...
3. LANGUAGE HANDLING: If the user asks for Telugu (తెలుగు), translate the response into fluent Telugu.
...
Source: Open-Meteo, ${currentDateStr} IST`;
  ...
  if (!responseText && toolResult && resolvedGeo) {
    responseText = synthesizePlainLanguageAdvisory(message, resolvedGeo, toolResult, currentDateStr);
  }

  // Ensure source citation is appended if not present
  if (weatherSnapshot && !responseText.toLowerCase().includes("source: open-meteo")) {
  ...
  // If user requested Telugu, or if global language is set to Telugu, generate or extract the Telugu translation
  if (isTeluguRequested) {
    const hasTeluguChar = /[\u0C00-\u0C7F]/.test(responseText);
    if (hasTeluguChar) {
      // The response already contains Telugu
      ...
      try {
        ...
        const teRes = await withTimeout(
          ai.models.generateContent({
            model: "gemini-2.5-flash",
            contents: `Translate this weather advisory directly into natural, clear Telugu (తెలుగు). Only provide the translation:\n\n${responseText}`,
          }),
          6000
        );
        if (teRes.text && teRes.text.trim()) {
          contentTelugu = teRes.text.trim();
        }
      } catch (e) {
        const fb = await translateToTeluguFallback(responseText);
        if (fb) contentTelugu = fb;
      }
  ...
  return res.json({
    reply: responseText || "Weather data is currently unavailable. Please verify your location and try again.",
    contentTelugu: contentTelugu || undefined,
    toolSummary,
    weatherSnapshot,
